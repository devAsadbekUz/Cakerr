-- Migration 84: Update Web Checkout RPC for Pickup System
-- Updates the create_order_v2 function to handle delivery_type and branch_id.
...
BEGIN
    ...
    SELECT 
        v_order_id, 
        CASE WHEN (item->>'product_id') = '' THEN NULL ELSE (item->>'product_id')::UUID END,
        (item->>'name'),
    ...
    IF p_promo_code_id IS NOT NULL THEN
        ...
        ON CONFLICT (user_id, promo_code_id) DO UPDATE SET usage_count = public.user_promo_usages.usage_count + 1;
